...
```mermaid
flowchart TD
    Z["hydrate settings from local SQLite snapshot — theme applies BEFORE first render (no system-theme flash)"] --> A["GestureHandlerRootView → ServerProvider → ToastHost"]
...
| `useSettingsStore` | preferences | **write-through persisted**: every change lands in the local SQLite blob (`cacheSetSettings`), and `AppLoader` hydrates it *before the first themed render* — so the theme never flashes from system to stored on cold start. The server copy (via `api.getSettings()`) still wins on refresh. `onboarded` defaults `true` so returning users never flash onboarding |
