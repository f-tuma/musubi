...
- app/
  ...
  - onboarding/        index (profile) → calendar → sync → share
  - invite/[token].tsx deep-link calendar preview + accept
</FileTree>

- **Onboarding recovery** survives OAuth round-trips via module-level state in `lib/onboardingState.ts`; `(tabs)/_layout.tsx` routes to it when `settings.onboarded === false`. The final share step offers a native Musubi calendar and opens its invite-link flow immediately after creation; it remains skippable.
...
- **Shared screen chrome** lives in `constants/layout.ts`: tab headers use one 56 dp height, while the icon-only tab bar derives its total height from a small top inset, a 44 dp item, and the device bottom safe area. The docked composer and root toast use that same calculation; do not reintroduce a hard-coded tab-bar height in either component.
...
Calendar-day UI that can remain mounted overnight uses `hooks/useCurrentDay.ts`. It schedules the next local midnight recursively (so DST is respected) and also refreshes on foreground, rather than waiting for an unrelated store update. The Home month marker uses a conditionally mounted rounded fill: changing only `backgroundColor` from transparent to accent can lose the existing radius during an Android Fabric style diff.
...
`Tap` assigns the button role automatically whenever it has `onPress`; specialized controls override it with `switch` or `radio` and expose `checked`, `selected`, `expanded`, `disabled`, and `busy` state where relevant. Icon-only actions need an explicit `accessibilityLabel`. Keep primary navigation and standalone controls at a 44 dp target; dense pills may preserve their small visual shape with vertical `hitSlop` instead of making the visible pill taller. The portal modal host uses `accessibilityViewIsModal`, and decorative dismiss backdrops are excluded from the accessibility tree so VoiceOver/TalkBack stays on meaningful sheet content.
