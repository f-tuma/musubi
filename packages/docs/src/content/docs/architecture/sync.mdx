...
Event writes use a different contract: Musubi persists the local event first,
then `pushEventToCalendars()` attempts each provider. Provider exceptions are
counted and logged but intentionally caught, so the API request can succeed
while a provider copy remains stale. No durable retry outbox currently exists.
...
| Event create/update/delete | Local DB, then provider(s) | Provider failure is logged; request continues |
