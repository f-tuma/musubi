...
- **`ios.associatedDomains`** in `app.config.ts` (`applinks:musubi.frgtn.dev`, `applinks:dev-musubi.frgtn.dev`) — EAS syncs the Associated Domains capability to the App ID at build time.
