...
interface ScheduleProps<T extends ICalendarEventBase> {
  ...
  itemSeparatorComponent?:
    | React.ComponentType<{ highlighted: boolean }>
    | null
    | undefined
  locale: string
...
function _Schedule<T extends ICalendarEventBase>({
  ...
  const eventStyles = React.useCallback(
    (event: T) => {
      const defaultEventStyle = { ...u['flex-column'], ...u['h-50'] }
      if (Array.isArray(eventCellStyle)) return [...[defaultEventStyle], ...eventCellStyle]
      if (typeof eventCellStyle === 'object') return { ...defaultEventStyle, ...eventCellStyle }
      if (typeof eventCellStyle === 'function') {
        const output = eventCellStyle(event)
        if (Array.isArray(output)) return [...[defaultEventStyle], ...output]
        if (typeof output === 'object') return { ...defaultEventStyle, ...output }
      }
  ...
  const renderMonthSeparator = (date: dayjs.Dayjs): JSX.Element => (
    <View style={{ width: '100%' }}>
      <Text style={[{ color: theme.palette.primary.main, textAlign: 'center', paddingVertical: 6 }, scheduleMonthSeparatorStyle]}>
  ...
  const renderFlatListItem = (eventGroup: T[], index: number): JSX.Element => {
    ...
    const isNewMonth = index === 0 || !dayjs(eventGroup[0].start).isSame(getItem[index - 1][0].start, 'month')
    ...
    return (
      <View style={[u.flex, { padding: 2, flexWrap: 'wrap' }]}>
        ...
        <View style={[u.flex, u['justify-center'], { width: '20%' }]}>
          <View
            style={[{ width: 60, height: 60, borderRadius: 30 }, u.flex, u['justify-center'], u['items-center'], u['flex-column-reverse']]}
            {...calendarCellAccessibilityProps}
          >
            <Text
              style={[
                {
                  color: shouldHighlight
                    ? stringHasContent(dayHeaderHighlightColor) ? dayHeaderHighlightColor : theme.palette.primary.main
            ...
            <Text
              style={[
                ...
                {
                  color: shouldHighlight
                    ? stringHasContent(weekDayHeaderHighlightColor) ? weekDayHeaderHighlightColor : theme.palette.primary.main
        ...
        <View style={[u.flex, u['flex-column'], { width: '75%' }]}>
          {eventGroup.map((event: T, index) => (
            <View style={[u['flex-1'], u['overflow-hidden'], { marginTop: 2, marginBottom: 2 }]} key={`${index}-${event.start}-${event.title}-${event.end}`}>
              <CalendarEvent
